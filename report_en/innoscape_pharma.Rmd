...
```{r context="data", cache = TRUE, echo=FALSE}
...
```{r, echo = F, child = file.path(knitr::opts_knit$get("output.dir"), "emp_output.Rmd"), optipng = '-o7'}
```
...
```{r, echo = F, child = file.path(knitr::opts_knit$get("output.dir"), "gva_output.Rmd"), optipng = '-o7'}
```
...
```{r, echo = F, child = file.path(knitr::opts_knit$get("output.dir"), "labprod_output.Rmd"), optipng = '-o7'}
```
...
Furthermore, our patent counts include only the so-called triadic patents. They are registered by all three major patent offices (European Patent Office, Japan Patent Office and the US Patent and Trademark Office) and protect the same invention. By taking such patents into account, home advantage and the influence of geographical locations are significantly reduced. However, note that our patent counts may differ somewhat from the <a href = https://stats.oecd.org/Index.aspx?DataSetCode=PATS_IPC# target = “_blank”> OECD </a> database in absolute values on triadic patent counts. This is because the OECD dataset is based on patent families while ours includes all distinct patents (i.e., the number of all non-equivalent patents) belonging to a triadic patent family. 
In the case of inventors coming from different countries, the geographical division is assigned and then summed as shares. For example, if one inventor comes from the USA and the other from Japan, the same patent will be assigned as 0.5 to the USA and 0.5 to Japan (for more information on how to determine the origins of patents see <a href = https://innoscape.ch/en/publications/cross-border-commuting-and-inventions-made-in-switzerland target = “_blank”> importance of cross-border commuters for inventions 'Made in Switzerland' </a>). 
...
```{r, echo = F, child  = file.path(knitr::opts_knit$get("output.dir"), "ctry_func.Rmd"), optipng = '-o7'}
```

### Regions

```{r, echo = F, child  = file.path(knitr::opts_knit$get("output.dir"),'regio_func.Rmd'), optipng = '-o7'}
```

### Continents

```{r, echo = F, child = file.path(knitr::opts_knit$get("output.dir"), "conti_func.RMD"), optipng = '-o7'}
```
...
```{r, echo = F, child = file.path(knitr::opts_knit$get("output.dir"), "citnetwork_output.Rmd"), optipng = '-o7'}
```
