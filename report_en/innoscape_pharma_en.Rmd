...
```{r context="data", cache=TRUE, echo = F}

# Load data on patent counts 
numpat_data <- xfun::cache_rds({readRDS("numpat_data.rds")})

# Load data on labor productivity 
labprod <- xfun::cache_rds({read.fst("labprod.fst")})

# Load data on employment / prepared in section_II/prep_report.R
ilo <- xfun::cache_rds({read.fst("ilo.fst")})
...
# Load citation-flow data  
citflow_ctry_back <- xfun::cache_rds({read.fst("citflow_ctry_back.fst")})
citflow_ctry_forw <- xfun::cache_rds({read.fst("citflow_ctry_forw.fst")})
