```{r, echo = F}
select_country <- distinct(dat_ii_world, Country, `Rank based on total patents`)  %>% na.omit()
...
## Create plot
renderPlotly({
  plot02_data_country <- filter(dat_ii_world, Country %in% selectedRow_ctry() & abs_rel %in% abs_rel_country())
 if(geo_share_ctry() == "abs"){
          ...
          # make the plot
                ...
                ggplotly(
                 ggplot(data = plot02_data_country, aes(x = p_year, y= share_geo, color = Country, shape = geo)) +
                 geom_point(alpha = 0.7) +
                 geom_line() +
                 # geom_text(aes(label = NOGA2digit), size = 4, check_overlap = FALSE, position = position_nudge(y = +0.1))+
                 xlab("Priority Year")+
