...
renderPlotly({
  plot02_data_conti <- filter(dat_ii, Continent %in% selectedRow_conti() & ipc_3 == "all" & abs_rel %in% abs_rel_conti())
...
# define the input panel for continents and IPCs: -------------------------------------------------------------------------
   ...
   choices = unique(na.omit(dat_ii$Continent)),
   selected = unique(na.omit(dat_ii$Continent)),
   options = list(
     `actions-box` = TRUE,
     size = 10,
...
## Create plot
renderPlotly({
  plot02_data_country <- filter(dat_ii, Country %in% selectedRow_ctry() & ipc_3 == "all" & abs_rel %in% abs_rel_country())
...
pickerInput(
   ...
   choices = unique(na.omit(dat_ii$Country)),
...
renderPlotly({
         ...
         # make the plot
                 abs <- ggplotly(
                 ggplot(data = plot2_data_ctry, aes(x = p_year, y= share_inv, color = Country, shape = ipc_3)) +
                 geom_point(alpha = 0.7) +
                 geom_line() +
                 # geom_text(aes(label = NOGA2digit), size = 4, check_overlap = FALSE, position = position_nudge(y = +0.1))+
                 xlab("\nPriority Year")+
...
## Create plot
renderPlotly({
  plot02_data_regio <- filter(dat_ii, Region %in% selectedRow_regio() & ipc_3 == "all" & abs_rel %in% abs_rel_regio)
...
pickerInput(
   ...
   choices = unique(na.omit(dat_ii$Region)),
...
renderPlotly({
         # make the plot
                 abs <- ggplotly(
                 ggplot(data = plot2_data(), aes(x = p_year, y= share_inv, color = Region, shape = ipc_3)) +
                 geom_point(alpha = 0.7) +
                 geom_line() +
                 # geom_text(aes(label = NOGA2digit), size = 4, check_overlap = FALSE, position = position_nudge(y = +0.1))+
                 xlab("Priority Year")+
