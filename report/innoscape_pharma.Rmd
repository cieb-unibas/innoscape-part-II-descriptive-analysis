...
dat_ii <- setDT(dat_ii)[, ges_pat_geo := sum(share_inv[abs_rel == "abs"]), .(geo)]
...
dat_ii_world <- filter(dat_ii_world, p_year < 2016)
dat_ii_world <- setDT(dat_ii_world)[, ges_pat_geo := sum(share_inv[abs_rel == "abs"]), .(geo)]
dat_ii_world <- setDT(dat_ii_world)[, `Rank based on total patents` := dense_rank(desc(ges_pat_geo)), .(geo_level)]
dat_ii_world <- as.data.frame(dat_ii_world)
...
```{r, echo = F, child  = file.path(knitr::opts_knit$get("output.dir"), "ctry_func_world_class.Rmd")}
```
