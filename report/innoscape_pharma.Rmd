...
```{r, echo = F}
# Load data
dat_ii <- readRDS("num_pat_16.rds") %>% data.frame() %>% filter(n_ipc > 99)
...
renderPlotly({
       ...
       # if(input$CONTI_ABS_REL == "rel_1990"){
       #           world <- ggplotly(
       #           ggplot(data = plot2_data_conti, aes(x = p_year, y= share_inv, color = geo, shape = ipc_main)) +
       #           geom_point(alpha = 0.7) +
       #           geom_line() +
       ...
       #             
       #           world <- ggplotly(
       #           ggplot(data = plot2_data_conti, aes(x = p_year, y= 1/share_inv, color = geo, shape = ipc_main)) +
       #           geom_point(alpha = 0.7) +
       #           geom_line() +
       #           # geom_text(aes(label = NOGA2digit), size = 4, check_overlap = FALSE, position = position_nudge(y = +0.1))+
       #           xlab("\nPriority Year")+
