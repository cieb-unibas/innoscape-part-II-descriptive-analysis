...
actionButton("geo_share_conti", "Absolute value / share")
...
renderPlotly({
  plot02_data_conti <- filter(dat_ii, Continent %in% selectedRow_conti()  & abs_rel %in% abs_rel_conti())
  if(geo_share_conti() == "abs"){
    # make the plot
    ggplotly(
      ggplot(data =   plot02_data_conti, aes(x = p_year, y= share_inv, color = geo, shape = geo)) +
    ...
    ggplotly(
      ggplot(data =   plot02_data_conti, aes(x = p_year, y= share_geo, color = geo, shape = geo)) +
        geom_point(alpha = 0.7) +
        geom_line() +
        # geom_text(aes(label = NOGA2digit), size = 4, check_overlap = FALSE, position = position_nudge(y = +0.1))+
        xlab("Priority Year")+
