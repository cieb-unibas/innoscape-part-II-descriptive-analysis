...
renderPlotly({
    ...
    ggplotly(
      ggplot(data =   plot02_data_conti(), aes(x = p_year, y= share_inv, color = geo, shape = geo, label = Geo)) +
        ...
        guides(color = FALSE)+
     scale_color_manual(values = colorRampPalette(brewer.pal(8, "RdBu"))(101)[c(1, 5, 15, 80, 90, 101)]) +
  scale_x_continuous(breaks = seq(min(plot02_data_conti()$p_year, na.rm = T), max(plot02_data_conti()$p_year, na.rm = T), by = 5)) +
      ...
      layout(xaxis = list(fixedrange=T, rangeslider = list(type = "date", visible=T, thickness=0.1)
                          )) %>%
