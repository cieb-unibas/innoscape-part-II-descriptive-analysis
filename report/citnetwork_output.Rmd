...
# (g) Creating Network Plot
  visNetwork(nodes, edges, height = "90%",  width = "90%", main = paste("Knowledge flows from other technologies to pharma")) %>%
...
# (g) Creating Network Plot
  visNetwork(nodes_2, edges_2, height = "100%",  width = "85%", main = paste("Knowledge flows from pharma to other technologies")) %>%
      visIgraphLayout(layout = "layout_nicely") %>%
      visEvents(type = "once", startStabilizing = "function() {
               this.moveTo({scale:0.5})}") %>%
      visPhysics(stabilization = FALSE) %>%
