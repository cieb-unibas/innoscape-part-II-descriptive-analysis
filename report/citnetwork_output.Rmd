...
# https://stackoverflow.com/questions/40279052/coordinates-of-equally-distanced-n-points-on-a-circle-in-r
  eq_spacing <- function(r = 1, data){
    # data: back or forward citations
  coord <- unique(data$tech_field_cited)
...
# Create coordinates for backward citations
 citflow_ctry_back <- eq_spacing(r = 1, data = citflow_ctry_back) 

# Create coordinates for forward citations
 citflow_ctry_forw <- eq_spacing(r = 1, data = citflow_ctry_forw) 
...
# (a) subset the data according to the input and store it for determining nodes and edges:
  ...
  nodes<- citflow_ctry_back %>% filter(group %in% input$group, 
...
# (b) Creating NODES from the reactive (filtered) data
  ...
  nodes$label<- ifelse(nodes$label == "Pharmaceuticals", paste0(input$cit_ctry), nodes$label)
...
# (e) Define different colors for different regions
  nodes <- mutate(nodes, value = ifelse(x == 0 & y == 0, 1, value_nodes), color = case_when(group == "All" ~ "#F8766D",
                                                                                            group == "Europe" ~ "#00BFC4",
                                                                                            group == "Americas" ~ "#00BBDB", 
                                                                                            group == "Asia" ~ "#7CAE00",
                                                                                            group == "Domestic" ~ "#C77CFF"),
...
# (g) Creating Network Plot
  visNetwork(nodes, edges, height = "130%",  width = "130%", main = paste("Knowledge flows to pharma")) %>%
    ...
    visLegend(position = "right", useGroups = F, addNodes = ledges, zoom = F) %>%
...
# (a) subset the data according to the input and store it for determining nodes and edges:
  ...
  nodes_2 <- citflow_ctry_forw %>% filter(group %in% input$group, 
                                   ctry_cited %in% input$cit_ctry,
                                   p_year_citing %in% input$p_year_citing)
  edges_2 <- nodes_2
...
# (b) Creating NODES from the reactive (filtered) data
  ...
  nodes_2$label<- ifelse(nodes_2$label == "Pharmaceuticals", paste0(input$cit_ctry), nodes_2$label)
...
# (e) Define different colors for different regions
  nodes_2 <- mutate(nodes_2, value = ifelse(x == 0 & y == 0, 1, value_nodes), color = case_when(group == "All" ~ "#F8766D",
                                                                                            group == "Europe" ~ "#00BFC4",
                                                                                            group == "Americas" ~ "#00BBDB", 
                                                                                            group == "Asia" ~ "#7CAE00",
                                                                                            group == "Domestic" ~ "#C77CFF"),
...
# (g) Creating Network Plot
  visNetwork(nodes_2, edges_2, height = "130%",  width = "130%", main = paste("Knowledge flows from pharma to other technologies")) %>%
      visIgraphLayout(layout = "layout_nicely") %>%
      visEvents(type = "once", startStabilizing = "function() {
               this.moveTo({scale:0.5})}") %>%
      visPhysics(stabilization = FALSE) %>%
