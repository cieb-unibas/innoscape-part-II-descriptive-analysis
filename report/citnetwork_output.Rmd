...
# (3b) Input panels for second citation country
    pickerInput(inputId = "cit_ctry_2",
              label = "Citations by (for comparison)", 
              choices= unique(citflow_ctry$ctry_cited),
...
# (g) Creating Network Plot
  visNetwork(nodes, edges, height = "130%",  width = "130%", main = paste(input$cit_ctry, "Pharmaceuticals")) %>%
...
# (b) Creating NODES from the reactive (filtered) data
  ...
  nodes_2$label<- ifelse(nodes_2$label == "Pharmaceuticals", paste0(input$cit_ctry_2), nodes_2$label)
...
# (g) Creating Network Plot
  visNetwork(nodes_2, edges_2, height = "130%",  width = "130%", main = paste(input$cit_ctry_2, "Pharmaceuticals")) %>%
      visIgraphLayout(layout = "layout_nicely") %>%
      visEvents(type = "once", startStabilizing = "function() {
               this.moveTo({scale:0.5})}") %>%
      visPhysics(stabilization = FALSE) %>%
