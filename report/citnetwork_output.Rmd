...
nodes <- eq_spacing(r = 1, data = citflow_ctry_back) 
edges <- nodes
...
# (g) Creating Network Plot
  visNetwork(nodes, edges, height = "100%",  width = "100%", main = paste("Knowledge flows to pharma")) %>%
...
# Create coordinates for forward citations used in 2nd plot
 nodes_2 <- eq_spacing(r = 1, data = citflow_ctry_forw) 
 edges_2 <- nodes_2
...
# (g) Creating Network Plot
  visNetwork(nodes_2, edges_2, height = "100%",  width = "90%", main = paste("Knowledge flows from pharma to other technologies")) %>%
      visIgraphLayout(layout = "layout_nicely") %>%
      visEvents(type = "once", startStabilizing = "function() {
               this.moveTo({scale:0.5})}") %>%
      visPhysics(stabilization = FALSE) %>%
